...
def evaluate_agents(agent_types=AGENT_TYPES, training_results_path="training_results", episodes=1000, outdir="test_results"):
    ...
    for agent_type in agent_types:
        ...
        for i in tqdm(range(number_of_agents)):
            agent.load(f"{sorted_ckpt_dirs[i]}/{ckpt_filename}")
...
def collect_qualitative_examples(agent_types=AGENT_TYPES,
    ...
    for a in agent_types:
        ...
        agent.show_progress(env=env, episodes=num_of_examples, save_gif=True, outdir=os.path.join(outdir, a))
    ...
    for ckpt in ckpt_dirs:
        ...
        # Save the GIFs to outdir
        agent.show_progress(env=env, episodes=1, save_gif=True, outdir=os.path.join(outdir, "DQN_learning_progression"))
